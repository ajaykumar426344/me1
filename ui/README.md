...
Open [http://35.213.148.83:3000](http://35.213.148.83:3000) to view it in the browser. (You should also run the ConsoleMe Python backend to see data)

ConsoleMe UI uses `proxy` in `package.json` file to proxy api backend requests to [http://35.213.148.83:8081](http://35.213.148.83:8081) for local development.
...
You can locally test this by accessing `http://35.213.148.83:8081` to see the actual build output served from ConsoleMe Tornado server.
